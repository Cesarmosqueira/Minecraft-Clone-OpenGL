...
void main() {
    float attenuation = attenuation_formula(distance(vec3(xyz.x, xyzBlock.y-1, xyz.z), xyzBlock));
    ...
    // total (phong)
	vec3 result    = (ambient + diffuse + specular) * vec3(CoordColor);
	color          = result;
}
